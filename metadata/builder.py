...
# =============================================================================
# 4d. Outputs step -- which artifacts (beyond the always-generated
#     benchmark description AND review report) this run should produce.
# =============================================================================
#
# Two artifacts are optional: the dataset-provenance sidecar
# (GraphBuilder.build_dataset_graph(), written by _build_impl() below) and
# the Snakefile (generated by describe_benchmark.py, outside this module).
# The review.md report (produced by show_description.py, also outside this
# module) is NOT one of the choices here -- describe_benchmark.py always
# generates it in-process right alongside the benchmark description itself,
# the same way it always writes benchmark.jsonld. This module only
# decides+persists the dataset/snakefile selection and controls its own
# piece of it (the dataset sidecar); describe_benchmark.py reads the
# resolved selection back out of build()'s returned stats dict to decide
# whether to also run the Snakefile step.

DEFAULT_OUTPUTS: dict[str, bool] = {"dataset": True, "snakefile": True}

_OUTPUT_PRESET_ALIASES: dict[str, dict[str, bool]] = {
    "standard": {"dataset": True, "snakefile": True},
    "all": {"dataset": True, "snakefile": True},
    "snakefile": {"dataset": False, "snakefile": True},
    "dataset": {"dataset": True, "snakefile": False},
    "description-only": {"dataset": False, "snakefile": False},
    "none": {"dataset": False, "snakefile": False},
}


def parse_outputs_arg(raw: str) -> dict[str, bool]:
    """Parse --outputs' string form: a preset name (see
    _OUTPUT_PRESET_ALIASES -- 'standard'/'all', 'snakefile', 'dataset',
    'description-only'/'none') or an explicit comma-separated list of
    'dataset'/'snakefile' (e.g. 'dataset,snakefile'). Raises ValueError
    with a message naming what was actually typed on anything else -- the
    CLI caller turns that into a sys.exit(). The review report isn't one
    of the choices here -- it's always generated, same as the benchmark
    description itself.
    """
    ...
    return {k: (k in keys) for k in valid}
...
def _resolve_outputs_selection(args: argparse.Namespace, module_dir: Path) -> dict[str, bool]:
    ...
    if not args.skip_review:
        ...
        filenames = {"dataset": "<name>_dataset.jsonld", "snakefile": "Snakefile"}
        while True:
            ...
            if _confirm_outputs_preview(selection, filenames):
                break
            current = selection  # re-open the picker seeded with the last choice
        save_outputs_config(module_dir, selection)
        return selection
...
def _plain_outputs_prompt(current: dict[str, bool]) -> dict[str, bool]:
    ...
    print("  Benchmark description   (always generated)")
    print("  Review report           (always generated)")
    ...
    while True:
        ...
        if not raw:
            return {k: (k in checked) for k in keys}
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--outputs", type=str, default=None,
                     help="Which artifacts to generate, beyond the benchmark description and review "
                          "report (both always generated). Either a preset -- 'standard' (dataset + "
                          "snakefile, the default), 'snakefile' (snakefile only, no dataset), "
                          "'dataset' (dataset only, no snakefile), 'description-only'/'none' (neither) "
                          "-- or an explicit comma-separated list of 'dataset', 'snakefile' (e.g. "
                          "'dataset,snakefile'). Default: reuse the selection saved from this module's "
                          "last run (.benchmantic_outputs.json), prompting interactively (real "
                          "terminal only) the first time, or 'standard' with --skip-review / no real "
                          "terminal.")
    return ap
...
def _build_impl(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _print_step_header(3, "Review semantic annotations")
    if not args.verbose:
    ...
    _print_step_header(4, "Select outputs")
