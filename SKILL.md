...
`<module_dir>` can be the exact benchmark folder or a repository checkout containing one. This walks you through reviewing the AI-inferred metadata (edit any field, or confirm as final), writes `benchmark.jsonld` and a packaged `Snakefile` to `outputs/<software-name>/`, renders a Markdown review table, and validates the result — exiting non-zero if verification fails. For CI/non-interactive use, add `--skip-review` (this step needs a real terminal). Skip individual pipeline steps with `--skip-show` / `--skip-check`.
...
2. **Infer** (`ai.cache`, `ai.inference`) — for candidates not already cached, asks the configured LLM to infer each one's semantic name, datatype, QUDT unit, and quantity kind, using the source code, `getParam<Type>()` call-site hints, and any relevant prior human corrections (`ai.corrections`) as context. Retries up to 3 times before failing (or falling back to a low-confidence placeholder with `--fallback-on-error`).
3. **Review** (`review.py`) — prints a table of every parameter/metric (name, datatype, unit, quantity kind, confidence). Enter a row number to edit any field, or Enter to finish. Rows below `--review-confidence-threshold` (default `0.7`) are flagged with `!` and block a plain confirm until fixed or explicitly accepted by typing `accept`. Skipped entirely with `--skip-review`. Whatever a reviewer changes is recorded by `ai.corrections` and offered back to the LLM as guidance for similarly named/typed items in *other* benchmarks.
4. **Build** (`metadata.builder`) — wraps each reviewed value plus the scraped manifest info into typed nodes inside one metadata4ing `@graph`.
5. **Validate & Generate** (`metadata.builder`, `snakefile.generator`, `snakefile.renderer`) — checks the graph against the RO-Crate 1.1 profile, then writes `benchmark.jsonld` and a packaged `Snakefile` to `outputs/<software-name>/`.

Review and verify the result downstream of generation:

- `show_description.py benchmark.jsonld` renders it as Markdown tables — including manifest fields (license, authors, dependencies) and resolved case values the interactive review step doesn't cover — saved to `review.md` as a durable record.
...
- `review.py` — interactive CLI review/edit step, run between inference and graph-build.
...
- `ai/` — `prompts.py`, `inference.py`, `validation.py`, `cache.py`, `corrections.py` — LLM-based semantic metadata inference and its cross-benchmark corrections memory.
